...
extension MKMapView {
    ...
    private func toPixelSpaceY(fromLatitude latitude: Double) -> Double {
        if latitude == 90.0 {
            ...
            let innerCalc = (1 + sinf(Float(latitude * Double.pi / 180.0)))
            let outerCalc = (1 - sinf(Float(latitude * Double.pi / 180.0)))
    ...
    private func toLatitude(fromPixelSpaceY pixelY: Double) -> Double {
        ...
        return (Double.pi / 2.0 - 2.0 * innerCalc) * 180 / Double.pi
    }
